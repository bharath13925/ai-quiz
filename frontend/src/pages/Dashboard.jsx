...
const TOPIC_META = {
  graphs: { icon: '🔗', accent: '#22d3ee', border: 'rgba(6,182,212,0.3)', gradFrom: 'rgba(6,182,212,0.15)', gradTo: 'rgba(6,182,212,0.03)', bar: '#06b6d4', btnFrom: '#06b6d4', btnTo: '#0891b2', glow: 'rgba(6,182,212,0.2)' },
  arrays: { icon: '📊', accent: '#60a5fa', border: 'rgba(59,130,246,0.3)', gradFrom: 'rgba(59,130,246,0.15)', gradTo: 'rgba(59,130,246,0.03)', bar: '#3b82f6', btnFrom: '#3b82f6', btnTo: '#2563eb', glow: 'rgba(59,130,246,0.2)' },
  dbms:   { icon: '🗄️', accent: '#a78bfa', border: 'rgba(139,92,246,0.3)', gradFrom: 'rgba(139,92,246,0.15)', gradTo: 'rgba(139,92,246,0.03)', bar: '#8b5cf6', btnFrom: '#8b5cf6', btnTo: '#7c3aed', glow: 'rgba(139,92,246,0.2)' },
  os:     { icon: '💻', accent: '#34d399', border: 'rgba(16,185,129,0.3)', gradFrom: 'rgba(16,185,129,0.15)', gradTo: 'rgba(16,185,129,0.03)', bar: '#10b981', btnFrom: '#10b981', btnTo: '#059669', glow: 'rgba(16,185,129,0.2)' },
}

const DEFAULT_META = { icon: '📚', accent: '#94a3b8', border: 'rgba(148,163,184,0.3)', gradFrom: 'rgba(148,163,184,0.15)', gradTo: 'rgba(148,163,184,0.03)', bar: '#94a3b8', btnFrom: '#94a3b8', btnTo: '#64748b', glow: 'rgba(148,163,184,0.2)' }
...
const StatCard = ({ icon, label, value, accentColor, delay }) => {
  ...
  return (
    <div className={`group relative p-5 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-cyan-500/20 hover:bg-white/[0.04] transition-all duration-500 text-center overflow-hidden cursor-default ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
...
const TopicCard = ({ topic, onStart, onLeaderboard, index }) => {
  const m = TOPIC_META[topic.id] || DEFAULT_META
  ...
  return (
    <div className={`transition-all duration-700 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
      <div
        className="group relative p-6 rounded-2xl transition-all duration-500 overflow-hidden cursor-default"
        style={{
          background: `linear-gradient(135deg, ${m.gradFrom}, ${m.gradTo})`,
          border: `1px solid ${hovered ? m.border : 'rgba(255,255,255,0.06)'}`,
          transform: hovered ? 'translateY(-4px) scale(1.01)' : 'translateY(0) scale(1)',
          boxShadow: hovered ? `0 20px 60px ${m.glow}, 0 0 0 1px ${m.border}` : 'none',
      ...
      >
        {/* Glow orb */}
        <div className="absolute -top-8 -right-8 w-24 h-24 rounded-full blur-3xl transition-all duration-700"
          style={{ background: m.accent, opacity: hovered ? 0.2 : 0 }} />

        <div className="relative z-10">
          <div className="flex items-start justify-between mb-4">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl transition-transform duration-300"
              style={{
          ...
          <h3 className="font-black text-xl mb-1 transition-all duration-300" style={{ color: m.accent }}>{topic.label}</h3>
          <p className="text-slate-500 text-xs mb-5">AI-adaptive · 4 windows · XGBoost</p>

          <div className="flex gap-2">
            <button onClick={() => onStart(topic.id)}
              className="flex-1 py-2.5 rounded-xl text-sm font-bold text-white hover:opacity-90 transition-all duration-200 hover:shadow-lg active:scale-[0.98]"
              style={{ background: `linear-gradient(135deg, ${m.btnFrom}, ${m.btnTo})` }}>
              Start Quiz →
            </button>
            <button onClick={() => onLeaderboard(topic.id)} title="Leaderboard"
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      setUser(u)
      try {
        // Topics come ONLY from backend — only what has questions in DB
  ...
  return (
    <div className="min-h-screen bg-[#020817]">
      ...
      <header className="sticky top-0 z-40 bg-[#020817]/80 backdrop-blur-2xl border-b border-white/5">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3.5 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 cursor-pointer group" onClick={() => navigate('/')}>
            ...
            <span className="font-black text-lg text-white" style={{ fontFamily: "'Syne',sans-serif" }}>
          ...
          <div className="hidden md:flex items-center gap-1 bg-white/[0.03] border border-white/10 rounded-xl p-1">
            {[{ label: 'Overview', id: 'overview' }, { label: 'Topics', id: 'topics' }, { label: 'Rankings', id: 'rankings' }].map((tab) => (
              <button key={tab.id}
                onClick={() => { setActiveTab(tab.id); if (tab.id === 'rankings') navigate('/leaderboard/graphs') }}
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-10 relative z-10">
        {/* Welcome banner */}
        <div className="relative mb-10 p-6 rounded-3xl overflow-hidden"
          style={{
            background: 'linear-gradient(135deg,rgba(6,182,212,0.08),rgba(59,130,246,0.04),transparent)',
            border: '1px solid rgba(6,182,212,0.15)',
            animation: 'fadeUp 0.6s ease forwards',
          }}>
          <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-bl from-cyan-500/8 to-transparent rounded-bl-3xl pointer-events-none" />
          <div className="relative z-10 flex items-center justify-between flex-wrap gap-4">
            <div>
              ...
              <h1 className="font-black text-3xl md:text-4xl text-white" style={{ fontFamily: "'Syne',sans-serif" }}>
                Hey, {firstName}! 👋
              </h1>
              <p className="text-slate-400 mt-1">
                {topics.length > 0
                  ? `${topics.length} topic${topics.length > 1 ? 's' : ''} available. Choose one to start your adaptive quiz.`
                  : 'No topics found — seed questions via POST /api/questions/bulk'}
              </p>
            </div>
            <button onClick={() => navigate('/leaderboard/graphs')}
              className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-amber-500/30 bg-amber-500/10 text-amber-400 text-sm font-bold hover:bg-amber-500/20 hover:border-amber-400/50 transition-all duration-300 hover:scale-105 active:scale-95">
              🏆 View Rankings
            </button>
          </div>
        </div>
        ...
        {/* Topics — only from backend */}
        {topics.length > 0 && (
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-black text-xl text-white flex items-center gap-2" style={{ fontFamily: "'Syne',sans-serif" }}>
                <span className="w-1 h-5 bg-gradient-to-b from-cyan-400 to-blue-500 rounded-full" />
                Available Topics
              </h2>
              <span className="text-slate-600 text-xs">{topics.length} topic{topics.length !== 1 ? 's' : ''} in DB</span>
            </div>
        ...
        {topics.length === 0 && (
          <div className="mb-12 p-10 rounded-3xl bg-white/[0.02] border border-white/5 text-center"
            style={{ animation: 'fadeUp 0.6s ease forwards 0.2s', opacity: 0, animationFillMode: 'forwards' }}>
            <div className="text-5xl mb-4">📭</div>
            <h3 className="font-bold text-xl text-white mb-2">No Topics Available</h3>
            <p className="text-slate-500 text-sm">
              Add questions via{' '}
              <code className="text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded font-mono">POST /api/questions/bulk</code>
            </p>
          </div>
        )}
          ...
          <div>
            ...
            <div className="space-y-2">
              {history.map((attempt, i) => {
                ...
                const tm  = TOPIC_META[attempt.topic] || DEFAULT_META
                return (
                  <div key={i}
                    ...
                    style={{ animation: `fadeUp 0.5s ease forwards ${i * 60}ms`, opacity: 0, animationFillMode: 'forwards' }}>
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl flex-shrink-0 group-hover:scale-110 transition-transform"
                        style={{ border: `1px solid ${tm.border}`, background: 'rgba(255,255,255,0.04)' }}>
                        {tm.icon}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-3 ml-14 sm:ml-0">
                      ...
                      <div className="text-right">
                        <p className="font-black" style={{ color: tm.accent }}>{attempt.score?.toFixed(1) || '0'}</p>
                      ...
                      <div className="hidden sm:block w-16 h-1.5 rounded-full bg-white/5 overflow-hidden">
                        <div className="h-full rounded-full transition-all duration-1000"
                          style={{ width: `${Math.round(attempt.accuracy * 100)}%`, background: tm.bar }} />
                      </div>
                    </div>
                  </div>
                )
